...
export default function CasesClient({ initialCases, dbCategories }) {
  ...
  const filteredCases = initialCases.filter((caseItem) => {
    ...
    if (caseItem.categories && Array.isArray(caseItem.categories)) {
      return caseItem.categories.some(
        (cat) => normalize(cat) === activeNormalized,
      );
    }
            ...
            <>
              ...
              >
                {visibleCases.map((caseItem) => {
                  const description =
                    locale === "ar"
                      ? caseItem.descriptionAr || caseItem.description
                      : caseItem.description;

                  // Use category as alt text since title is removed
                  const altText =
                    caseItem.category ||
                    caseItem.categories?.[0] ||
                    "Case Image";
                  const allCategories =
                  ...
                  const isDetailed =
                    ...
                    (!caseItem.caseType &&
                      (() => {
                        ...
                        if (
                          caseItem.categories &&
                          Array.isArray(caseItem.categories)
                        ) {
                          return caseItem.categories.some(
                            (cat) =>
                              normalize(cat) ===
                  ...
                  let simpleImages = [];
                  if (!isDetailed) {
                    if (caseItem.coverImage)
                      simpleImages.push(caseItem.coverImage);
                  ...
                  return (
                    <div
                      key={caseItem.id}
                      className={`${styles.caseCard} ${!isDetailed ? styles.simpleCard : ""}`}
                    >
                      <div className={styles.imageWrapper}>
                        {isDetailed ? (
                          ...
                          caseItem.coverImage ? (
                            <Image
                              src={
                                caseItem.coverImage || "/images/placeholder.jpg"
                              }
                              alt={altText}
                              fill
                            ...
                            <ImageSlider
                              ...
                              afterImage={
                                caseItem.afterImage || caseItem.afterImageUrl
                              }
                            />
                          )
                      ...
                      {isDetailed ? (
                        ...
                        >
                          ...
                          <span className="btn-secondary" style={{ alignSelf: "flex-start", marginTop: "auto" }}>
                            {t("viewCase")}
                          </span>
                        </Link>
                      ) : (
                        <div
                          className={`${styles.caseInfo} ${styles.simpleCardInfo}`}
                        >
                          <div className={styles.categoriesWrapper}>
                            {allCategories.map((cat) => {
                              const catObj = dbCategories.find(
                          ...
                          <p className={styles.simpleCardDesc}>{description}</p>
                        </div>
                      )}
                    </div>
                  );
