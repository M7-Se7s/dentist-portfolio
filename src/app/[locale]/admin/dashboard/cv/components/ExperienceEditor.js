export default function ExperienceEditor({ experiences, setExperiences, styles }) {
  ...
  return (
    <div className={styles.flatBasicInfoCard}>
      ...
      <button type="button" onClick={addExperience} className={styles.btnDashed} style={{ marginBottom: '24px' }}>
...
const DynamicResponsibilitiesList = ({ exp, updateExperienceFields, styles }) => {
  ...
  return (
    <div style={{ marginTop: '16px', borderTop: '1px solid #E2E8F0', paddingTop: '16px' }}>
      ...
      {safeEN.length === 0 && (
        <p style={{ fontSize: '14px', color: 'var(--text-muted)', fontStyle: 'italic', marginBottom: '16px' }}>No responsibilities added yet. Click "+ Add Responsibility" below.</p>
      )}
      ...
      {safeEN.map((_, index) => (
        <div key={index} className={styles.bilingualRowDynamic} style={{ marginBottom: '12px', alignItems: 'center', backgroundColor: '#F8FAFC', padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
          <div style={{ flex: 1 }}>
            <input type="text" value={safeEN[index] || ''} onChange={(e) => handleChangeEN(index, e.target.value)} placeholder="Responsibility in English" style={{ width: '100%', padding: '8px', border: '1px solid #CBD5E1', borderRadius: '4px', margin: 0 }} />
          </div>
          <div style={{ flex: 1 }}>
            <input type="text" value={safeAR[index] || ''} onChange={(e) => handleChangeAR(index, e.target.value)} dir="rtl" placeholder="Responsibility in Arabic" style={{ fontFamily: 'var(--font-arabic)', width: '100%', padding: '8px', border: '1px solid #CBD5E1', borderRadius: '4px', margin: 0 }} />
          </div>
      ...
      <button type="button" onClick={handleAdd} className="btn-secondary" style={{ padding: '6px 12px', fontSize: '14px', marginTop: '8px' }}>+ Add Responsibility</button>
    </div>
  );
}
