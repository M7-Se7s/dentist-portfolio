...
export default function CVEditorPage() {
  ...
  const [basicInfoAr, setBasicInfoAr] = useState({
    name: "",
    title: "",
    location: "",
  });
  ...
  const [education, setEducation] = useState([
    {
      ...
      institution: "Faculty of Dental Medicine\nAl-Azhar University – Egypt",
      year: "Graduated: 2023",
    },
  ]);
  ...
  const [licensure, setLicensure] = useState([
    ...
    },
    {
      ...
      details:
        "Professional Rank: General Dentist (General Dentistry) | Profile Number: 26904058 | Status: Practicing | Expiry Date: 30/03/2028",
      detailsAr:
        "الرتبة المهنية: طبيب أسنان عام (طب الأسنان العام) | رقم الملف: 26904058 | الحالة: ممارس | تاريخ الانتهاء: 30/03/2028",
    },
  ]);
  ...
  const [experiences, setExperiences] = useState([
    {
      ...
      clinic: "Dr. Ahmed Saleh Dental Clinic",
    ...
    },
    {
      id: 2,
      role: "General Dentist",
      clinic: "Dr. Ali Naguib Dental Clinic",
    ...
    },
    {
      id: 3,
      role: "General Dentist",
      clinic: "Dr. Mostafa Yehia Dental Clinic",
    ...
    },
    {
      id: 4,
      role: "General Dentist",
      clinic: "Dr. Gamal Saad El-Din Dental Clinic",
    ...
    },
    {
      ...
      clinic:
        "Al-Azhar University Hospitals, Boulak Teaching Hospital, & Shibin El-Kom Teaching Hospital, Egypt",
      period: "October 2023 – October 2024",
      responsibilities:
        "Completed clinical rotations across multiple dental specialties including Endodontics, Oral Surgery, Prosthodontics, and Pedodontics.\nGained extensive hands-on experience in emergency dental care, infection control validation, and basic surgical maneuvers within high-volume educational and public health settings.",
    },
  ]);

  const defaultClinicalSkillsStr =
  ...
  const [clinicalSkills, setClinicalSkills] = useState(
    parseMarkdownToCategories(defaultClinicalSkillsStr),
  );
    ...
    async function fetchCV() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.coreCompetenciesAr)
            setCoreCompetenciesAr(data.coreCompetenciesAr);
          if (data.education) setEducation(data.education);
          if (data.licensure) setLicensure(data.licensure);
          if (data.experiences) {
            const exps = data.experiences;
            if (
              !exps.find((e) => e.role && e.role.includes("Intern Dentist"))
            ) {
              exps.push({
                id: Date.now(),
                role: "Intern Dentist (Rotational Internship)",
                ...
                clinic:
                  "Al-Azhar University Hospitals, Boulak Teaching Hospital, & Shibin El-Kom Teaching Hospital, Egypt",
                clinicAr:
                  "مستشفيات جامعة الأزهر، مستشفى بولاق الدكرور التعليمي، ومستشفى شبين الكوم التعليمي، مصر",
                period: "October 2023 – October 2024",
                periodAr: "أكتوبر 2023 – أكتوبر 2024",
                responsibilities:
                  "Completed clinical rotations across multiple dental specialties including Endodontics, Oral Surgery, Prosthodontics, and Pedodontics.\nGained extensive hands-on experience in emergency dental care, infection control validation, and basic surgical maneuvers within high-volume educational and public health settings.",
                responsibilitiesAr:
                  "إتمام دورات سريرية عبر تخصصات طب الأسنان المتعددة بما في ذلك علاج الجذور، جراحة الفم، الاستعاضة السنية، وطب أسنان الأطفال.\nاكتساب خبرة عملية واسعة في الرعاية الطارئة لطب الأسنان، التحقق من مكافحة العدوى، والمناورات الجراحية الأساسية في بيئات تعليمية وصحية عامة مزدحمة.",
              });
              exps.sort((a, b) => {
          ...
          if (data.clinicalSkills)
            setClinicalSkills(parseMarkdownToCategories(data.clinicalSkills));
          if (data.clinicalSkillsAr)
            setClinicalSkillsAr(
              parseMarkdownToCategories(data.clinicalSkillsAr),
            );
  ...
  const handleAutoTranslate = async () => {
    if (
      !confirm(
        "This will auto-translate all English fields into Arabic. Existing Arabic text may be overwritten. Continue?",
      )
    )
      return;
    ...
    try {
      ...
      setMessage(
        "Translation complete! Please review the Arabic fields and hit Save.",
      );
    ...
    try {
      ...
      await setDoc(
        docRef,
        {
          ...
          coreCompetenciesAr: coreCompetenciesAr.filter(
            (s) => s?.trim() !== "",
          ),
          education,
          licensure: licensure,
      ...
      await triggerRevalidation(["/[locale]/cv", "/[locale]"], "page");
    ...
    >
      ...
      >
        <div>
          ...
          >
            Manage your Curriculum Vitae data directly synced to your website.
          </p>
        </div>
