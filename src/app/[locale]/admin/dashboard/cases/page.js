        ...
        <div className={styles.caseCardGrid}>
          ...
          {activeUploads &&
            activeUploads.map((upload) => (
              ...
              >
                {/* Part 1: Full Bleed Image Area */}
                <div className={styles.caseCardImageArea}>
                  {(upload.beforeImageFile && upload.afterImageFile) ? (
                    <div style={{ width: '100%', height: '100%' }}>
                      <ImageSlider 
                        beforeImage={URL.createObjectURL(upload.beforeImageFile)} 
                        afterImage={URL.createObjectURL(upload.afterImageFile)} 
                      />
                    </div>
                  ) : (upload.coverImageFile || upload.beforeImageFile) ? (
                    <img 
                      src={URL.createObjectURL(upload.coverImageFile || upload.beforeImageFile)} 
                      alt="Thumbnail" 
                    />
                  ) : (
                    <div className={styles.caseAvatarFallback}>
                      {(upload.formData?.categories?.[0] || "CA").substring(0, 2).toUpperCase()}
                    </div>
                  )}

                  {/* Floating Action Buttons / Status */}
                ...
                {/* Part 2: Content Area */}
                <div className={styles.caseCardContentArea}>
                  <h3 className={styles.caseCardTitle}>
                    {upload.formData?.categories?.[0] || "Uploading Case..."}
                  </h3>

                  <div className={styles.caseCardDetails} style={{ marginTop: "auto" }}>
                    <div
                      style={{
                        width: "100%",
                ...
                <div key={caseItem.id} className={styles.caseCardItem}>
                  ...
                  <div className={styles.caseCardImageArea}>
                    {caseItem.beforeImage && caseItem.afterImage ? (
                      <div style={{ width: '100%', height: '100%' }}>
                        <ImageSlider beforeImage={caseItem.beforeImage} afterImage={caseItem.afterImage} />
                      </div>
                    ) : caseItem.coverImage || caseItem.beforeImage ? (
                      <img 
                        src={caseItem.coverImage || caseItem.beforeImage} 
                        alt={caseItem.category || "Case thumbnail"} 
                      />
                    ) : (
                      <div className={styles.caseAvatarFallback}>
                    ...
                    {/* Floating Action Buttons (Split Left/Right) */}
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', right: '1rem', zIndex: 10, display: 'flex', justifyContent: 'space-between' }}>
                      {/* Left Side: Delete */}
                      <div className={styles.caseAdminActions}>
                        <button
