...
export default function ImageSlider({
  ...
  // Pointer events fallback/support
  ...
  const handlePointerMove = (e) => {
    ...
    if (touchState.current.isHorizontal === null) {
      const dx = Math.abs(clientX - touchState.current.startX);
      const dy = Math.abs(clientY - touchState.current.startY);
      if (dx > 5 || dy > 5) touchState.current.isHorizontal = dx > dy;
    }
    if (touchState.current.isHorizontal) {
      const rect = sliderRef.current.getBoundingClientRect();
      const x = clientX - rect.left;
    ...
    >
      ...
      >
        ...
        <Image
          ...
          unoptimized={true}
        />
        <span className={styles.labelBefore} style={{ zIndex: 20 }}>
          Before
        </span>
      </div>
      ...
      >
        ...
        <Image
          ...
          unoptimized={true}
        />
        <span className={styles.labelAfter} style={{ zIndex: 20 }}>
          After
        </span>
      </div>
